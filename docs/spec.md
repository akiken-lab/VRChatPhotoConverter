...
### 2.4 常駐とウィンドウ挙動
1. ウィンドウを最小化すると、アプリはタスクバーから消えてトレイに格納される。
2. ウィンドウを閉じても、既定動作では終了せずトレイに格納される。
3. 完全終了はトレイメニューの「終了」から行う。
4. 最小化時と「閉じる」によるトレイ格納時は、トレイ通知で挙動を案内する。

### 2.5 監視方式と負荷特性
監視は常時ディレクトリ走査ではなく、プロセス終了検知ベースで動作する。

1. 一次方式
1. `ManagementEventWatcher` で `Win32_ProcessStopTrace` を購読する。
2. 対象 EXE 名に一致した終了イベント受信時のみ変換処理へ進む。

2. フォールバック方式
1. WMI が使えない環境では 2 秒間隔のタイマー監視へ切り替える。
2. ポーリングではプロセス一覧を比較し、対象 EXE の終了差分だけを検知する。

3. 負荷評価
1. 待機中は PNG 列挙、JPEG 変換、履歴 DB 更新を行わない。
2. そのため待機時負荷は低い。
3. ただし WMI イベント購読や 2 秒周期タイマーは存在するため、厳密には「ゼロ負荷」ではない。
