...
## GitHub / BOOTH 配布で案内しておくとよい点
- インストーラーはありません
- 配布形式は zip です
- 展開先フォルダを削除しても、設定と履歴は `%LOCALAPPDATA%` に残ります
- 完全に削除したい場合は展開フォルダに加えて `%LOCALAPPDATA%\VRCJpegAutoGenerator` も削除してください
- 自動更新機能はありません。更新時は新しい zip へ置き換えます
